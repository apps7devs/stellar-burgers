...
// ############################################ THREE TEST ############################################
  it('Test 3: modal ingredient open/close actions', () => {
    ...
    cy.get('[data-testid="cy-close-modal-button"]').should('exist')
    cy.url().should('include', '/ingredients/643d69a5c3f7b9001cfa093d')
    cy.get('[data-testid="cy-ingredient-details-name"]')
      .should('exist')
      .contains('Флюоресцентная булка R2-D3')
    cy.get('[data-testid="cy-close-modal-button"]').should('exist').click()
    cy.get('[data-testid="cy-close-modal-button"]').should('not.exist')
